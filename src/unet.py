...
from example_code.tf_aerial_images import extract_data, extract_labels
...
def down_sample(data, n_filter, filter_size, in_channel, leaky, dropout):
    conv = conv_block(data, filter_size, n_filter, in_channel, leaky)
...
def up_sample(data, n_filter, filter_size, in_channel, out_img_size, leaky, dropout):
    conv = conv_block(data, filter_size, n_filter, in_channel, leaky)
    up = tf.nn.conv2d_transpose(conv,
                                tf.Variable(
                                    tf.truncated_normal(
                                        [filter_size, filter_size, int(n_filter / 2), n_filter])),
                                output_shape=[BATCH_SIZE, out_img_size, out_img_size, int(n_filter / 2)],
                                strides=[1, 2, 2, 1],
                                padding='SAME')
    if dropout is not None:
        up = tf.nn.dropout(up,
...
def unet(data, n_filter, filter_size, in_channel, leaky=False, dropout=None):
    ...
    up6 = tf.nn.conv2d_transpose(conv5,
                                 ...
                                 output_shape=[BATCH_SIZE, int(IMG_PATCH_SIZE / 8), int(IMG_PATCH_SIZE / 8),
                                               n_filter * 8],
                                 strides=[1, 2, 2, 1],
                                 padding='SAME')
    ...
    up7 = up_sample(up6, n_filter * 8, filter_size, n_filter * 16, int(IMG_PATCH_SIZE / 4), leaky, dropout)
    up7 = tf.concat([up7, conv3], axis=3)

    up8 = up_sample(up7, n_filter * 4, filter_size, n_filter * 8, int(IMG_PATCH_SIZE / 2), leaky, dropout)
    up8 = tf.concat([up8, conv2], axis=3)

    up9 = up_sample(up8, n_filter * 2, filter_size, n_filter * 4, IMG_PATCH_SIZE, leaky, dropout)
